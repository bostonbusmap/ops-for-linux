...
gboolean download_memory_start(gpointer data) {
  return DownloadMemory(data, start_global, length_global);
}

gboolean download_memory_store_filename(GtkFileSelection* file_selection_box) {
  GError *error;
...
gboolean download_memory_confirmed(double_widget* d_w) {
...
gboolean DownloadMemory(const char* filename, unsigned long start, unsigned long length) {
